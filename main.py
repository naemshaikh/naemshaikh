...
def _auto_paper_sell(address, reason, sell_pct=100.0):
    ...
    if TRADE_MODE == "real":
        ...
        # FIX v107: FM_BC source → getTokenInfo + BC sell first, PC fallback
        # Non-FM (graduated) → seedha PC, no extra RPC call
        if "FM_BC" in _source:
            ...
            if not _graduated:
                ...
                if _real_sell.get("success"):
                    ...
                    print(f"🔁 [FM v107] BC sell TX sent ({address[:10]}) — tracker se confirm hone pe state close hogi")
                    return
                else:
                    _fail_err_v97 = _real_sell.get("error", "?")
                    print(f"❌ [FM v107] BC sell failed — PC fallback: {_fail_err_v97}")
                    _real_sell = real_sell_token(address, sell_pct, _buy_tax_s, _sell_tax_s)
            else:
                # Graduated → PC sell
                print(f"🎓 [FM v107] Token graduated — PC sell: {address[:10]}")
                _real_sell = real_sell_token(address, sell_pct, _buy_tax_s, _sell_tax_s)
        else:
            # Non-FM token → seedha PC, no RPC overhead
            _real_sell = real_sell_token(address, sell_pct, _buy_tax_s, _sell_tax_s)
                        ...
                        if len(_fm_funds_hist) >= 2:
                            ...
                            if _f_drop >= 20:
                                if tp_sold > 0:
                                    ...
                                    _instant_dump = True
                                    print(f"🚨 [FM] Funds -{_f_drop:.1f}% instant dump (pre-TP): {addr[:10]}")
                            elif _f_drop >= 5:
...
def _fm_confirm_close(token_addr, sell_pct, reason, tx_hash_hex):
    ...
    try:
        ...
        if sell_pct >= 100.0:
            # Full close — paper_balance aur banked update karo
            sess["paper_balance"] = round(sess.get("paper_balance", 5.0) + return_bnb, 6)
            auto_trade_stats["auto_pnl_total"] += pnl_pct
            auto_trade_stats["total_auto_sells"] += 1
            _banked = pos.get("banked_pnl_bnb", 0.0)
            pos["banked_pnl_bnb"] = round(_banked + pnl_bnb, 6)
        # FIX v93b: Partial sell (TP1/TP2/TP3) — _auto_paper_sell ne pehle hi
        # sahi size se banked_pnl_bnb + paper_balance update kar diya hai.
        # _fm_confirm_close pe size already reduced hoti hai (60%) — dobara add karne se
        # galat (24% = 60%*40%) amount add hota — skip karo.
        ...
        def _bc_track_and_parse(_th_hex, _t_addr, _t_name, _w3t, _min_f):
            ...
            def _retry_sell(_t_addr2, _w3r, _attempt_num):
                try:
                    ...
                    # FIX v93: sell_pct respect karo — pehle _bal_r (100%) sell hota tha
                    # TP1 (40%) TX revert → retry mein poora balance sell ho jaata tha
                    # sell_pct closure se milta hai — partial TP pe partial retry
                    _amt_r = int(_bal_r * sell_pct / 100)
                    if _amt_r <= 0:
                        print(f"✅ [FM BC] Calc amt 0 — skip retry"); return None
                    print(f"🔁 [FM v93] Retry sell_pct={sell_pct:.0f}% | bal={_bal_r} | amt={_amt_r}")
                    ...
                    _zero = "0x0000000000000000000000000000000000000000"
                    if _tv_r == 1:
                        _txr = _fc_r.functions.sellToken(0,Web3.to_checksum_address(_t_addr2),_amt_r,0,0,_zero
                            ).build_transaction({"from":_wc_r,"gas":400000,"gasPrice":_gp_r,"nonce":_nn_r,"chainId":56})
                    else:
                        _txr = _fc_r.functions.sellToken(0,Web3.to_checksum_address(_t_addr2),_wc_r,_amt_r,0,0,_zero
                            ).build_transaction({"from":_wc_r,"gas":400000,"gasPrice":_gp_r,"nonce":_nn_r,"chainId":56})
        ...
        def _check_genuine(price_history, funds_history, ub_history, price_samples, total_buys=0, block_wallets=None, wallet_amounts=None):
            ...
            if len(funds_history) >= 3:
                ...
                if recent_price_up and recent_vol_drop:
                    ...
                    score -= 1  # FIX v96: -2→-1, vol_flow_dead already same overlap cover karta hai
